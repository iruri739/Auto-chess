<template>
  ...
  <div id="Login">
    ...
    <el-dialog title="注册" :visible.sync="dialogFormVisible">
      <el-form :model="regform" ref="regform" :rules="rules">
        ...
        <el-form-item>
              ...
              <el-dialog
               title="《Superance协议》"
               :visible.sync="centerDialogVisibleq"
              width="70%"
               center>
              <span>
                1. 许可权利的授予。本《协议》授予您下列权利：《自走棋》网络游戏客户端软件的安装、运行。您可在许可生效的时间内将《自走棋》网络游戏客户端软件安装在自己使用的联网计算机上，并以客户端软件指定的方式运行本“软件产品”的一份副本。其他任何形式的未经许可的安装、使用、访问、显示、运行以及转让，都将被视为对《协议》的侵犯。
    ...
    <div class="sixteen"><span><p><strong>合理安排时间&nbsp;享受健康生活&nbsp;注意自我保护&nbsp;拒绝盗版游戏</strong></p></span></div>
  </div>
    ...
    return {
     ...
     centerDialogVisibleq: false,
      ...
      rules: {
        ...
        type: [
            {type: 'array', required: true, message:'请仔细阅读协议并确认', trigger: 'change' }
          ],
        typeq: [
            {type: 'array', required: true, message:'请仔细阅读协议并确认', trigger: 'change' }
          ],   
      ...
      this.$refs[regform].validate(valid => {
        if (valid) {
          if (
            ...
            this.$http.post('http://localhost:8888/user/register/',
           {
